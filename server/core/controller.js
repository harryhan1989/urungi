...
Controller.prototype.findAll = asyncDecorator(findAll);
Controller.prototype.findOne = asyncDecorator(findOne);
Controller.prototype.findOneForServer = asyncDecorator(findOneForServer);
Controller.prototype.create = asyncDecorator(create);
Controller.prototype.update = asyncDecorator(update);
Controller.prototype.remove = asyncDecorator(remove);

function asyncDecorator (f) {
    /*
    *   Widestage was initially built using only callbacks
    *
    *   This wrapper makes it possible to use promises for the core functions.
    *   Core functions return promises if no done argument is passed
    */

    return function (req, done) {
        if (done) {
            return f.bind(this)(req, done);
        } else {
            return new Promise((resolve, reject) => {
                f.bind(this)(req, resolve);
            });
        }
    };
}

function findAll (req, done) {
...
function findOneForServer (req, done) {
    if (!req.query.id) {
        done({result: 0, msg: "'id' is required."});
        return;
    }
